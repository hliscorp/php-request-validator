...
class Validator
{
    ...
    private function match(string $routeURI): void
    {
        ...
        if (sizeof($routes) == 0) {
            throw new Exception("Tag 'routes' missing or lacking 'route' subtags");
        }
        foreach ($routes as $info) {
            if (empty($info['url'])) {
                throw new Exception("Attribute 'url' is mandatory for 'route' tag");
            }
            $url = (string) $info['url'];
            if ($url == $routeURI) {
                $this->validate($info);
    ...
    private function validate(\SimpleXMLElement $info): void
    {
        ...
        if ($count == 0) {
            return;
        } // it is ok to have no rules set
        foreach ($tags as $tag) {
            $name = (string) $tag["name"];
            $validator = (string) $tag["validator"];
            if (!$name || !$validator) {
                throw new Exception("Attributes 'name' and 'validator' are mandatory for 'parameter' tags!");
            }
            ...
            } elseif ($mandatory === "0" && !isset($this->requestParameters[$name])) {
